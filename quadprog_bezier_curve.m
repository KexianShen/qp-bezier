...
beq = reshape([start_end_points;start_end_vel/n*T],[],1);
...
bieq = ones(8,1)*a_limit/n/(n-1)*T*T;

[p,fval] = quadprog(Q,b,Aieq,bieq,Aeq,beq);
ControlPoints = reshape(p,[],2);
...
if plot_flag
    ...
    subplot(5,2,[3 4]),plot(timeIndex,v_abs),ylabel('|v|');xlabel('t');
    subplot(5,2,5),plot(timeIndex,v(:,1)),ylabel('vx');xlabel('t');
    subplot(5,2,6),plot(timeIndex,v(:,2)),ylabel('vy');xlabel('t');
    subplot(5,2,[7 8]),plot(timeIndex,a_abs),ylabel('|a|');xlabel('t');
    subplot(5,2,9),plot(timeIndex,a(:,1)),ylabel('ax');xlabel('t');
    subplot(5,2,10),plot(timeIndex,a(:,2)),ylabel('ay');xlabel('t');
end